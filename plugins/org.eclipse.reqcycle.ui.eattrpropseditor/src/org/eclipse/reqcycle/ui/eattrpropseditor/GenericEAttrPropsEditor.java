...
/**
 * <p>
 * This is a generic SWT Composite that holds an {@link IEAttrPropsEditor}. When setting the attribute and optionally
 * the javaClass type of the attribute which is to be editing, this class GenericEAttrPropsEditor will instantiate the
 * correct IEAttrPropsEditor to use and show to the end user. The IEAttrPropsEditor which is used is search into the
 * available implementations. In order to set the attribute which will trigger this class to use the correct properties
 * editor, just & simply use the {@link #init(EAttribute, String)} method.
 * </p>
 * <p>
 * <b>NOTE :</b> It is then by far easier to use this class instead of the trying to use directly a IEAttrPropsEditor
 * which would require to know the type of the attribute first and also know whether or not an IEAttrPropsEditor is
 * available for editing that attribute.
 * </p>
 * 
...
public class GenericEAttrPropsEditor extends Composite {
     ...
     * 
     * @param eAttribute - The attribute to edit.
     * @param javaClassTypeName - The javaClassType of the attribute. Optional, and so may be <tt>null</tt>. If not
     *            <code>null</code>, then no further lookup will be done onto the attribute in order to retrieve its
    ...
    public void init(final EAttribute eAttribute, final String javaClassTypeName) {
        ...
        this.propsEditor = EAttrPropsEditorPlugin.getStructuralFeatureEditor(eAttribute, javaClassTypeName, this,
                getStyle());
