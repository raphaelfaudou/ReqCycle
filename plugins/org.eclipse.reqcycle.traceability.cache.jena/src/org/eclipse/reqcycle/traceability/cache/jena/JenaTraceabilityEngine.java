...
@Singleton
public class JenaTraceabilityEngine extends AbstractCachedTraceabilityEngine {
	...
	protected void environmentClosed() {
		if (logger.isDebug(Activator.DEBUG_OPTIONS, Activator.getDefault())) {
			logger.trace("Jena is closing");
		}
		storage.save();
		storage.dispose();
		if (logger.isDebug(Activator.DEBUG_OPTIONS, Activator.getDefault())) {
			logger.trace("Jena closed, rdf saved : " + getRDFPath());
		}
	}

	public String getRDFPath() {
		return getCachePath() + "/rdfcache.rdf";
	}

	@PostConstruct
	public void postConstruct() {
		if (logger.isDebug(Activator.DEBUG_OPTIONS, Activator.getDefault())) {
			logger.trace("Jena Initialization");
		}
		storage = storageEngine.getStorage(getRDFPath());
		if (logger.isDebug(Activator.DEBUG_OPTIONS, Activator.getDefault())) {
			logger.trace("Jena is initialized");
		}
	}
	...
	protected Iterator<Pair<Link, Reachable>> doGetTraceability(
		...
		if (source != null && target != null) {
			...
			if (checkPath((Reachable) i.next(), target, i, result, current)) {
	...
	private boolean checkPath(Reachable s, StopCondition condition,
	...
	public void newUpwardRelation(Reachable container, Reachable source,
		...
		storage.newUpwardRelationShip(kind, container, source,
